      ...
      if (request.readyState == 4) {
        if (request.status == 200) {
          try {
            ...
            for (var src in config) {
              try {
                var re = new RegExp(src);
                if (re.test(location.href)) {
                  netacad.config = netacad.merge(netacad.config, config[src]);
                }
              } catch (e) {
                console.warn("invalid regular expression in " + url);
              }
            }
          } catch (e) {
...
netacad.views.ContentView = function(model, element) {
  ...
  this.element.parentNode.style.setProperty("min-width", this.minWidth + "px");
  this.element.parentNode.style.setProperty("min-height", this.minHeight + "px");
...
netacad.views.CourseMapView.prototype.handleChange = function(event) {
...
netacad.views.CourseIndexView.prototype.buildModuleList = function(modules, filter) {
  ...
  for (var i = 0; i < modules.length; i++) {
    ...
    if (module && filter(module)) {
       // <li><span class="module-title">Module 1<br/>Module Title</span>...</li>
       var li = this.document.createElement("li");
       var span = this.document.createElement("span");
       span.setAttribute("class", "module-title");
       span.appendChild(this.document.createTextNode(netacad.lang.get("Module").replace("#", module.li)));
       span.appendChild(this.document.createElement("br"));
       span.appendChild(this.document.createTextNode(module.title));
       li.appendChild(span);
       li.appendChild(this.buildSectionList(module.children, filter));
       ul.appendChild(li);
       empty = false;
    }
  }
  if (empty) {
    var li = this.document.createElement("li");
    li.setAttribute("class", "message");
    li.appendChild(this.document.createTextNode(netacad.lang.get("NoItems")));
    ul.appendChild(li);
  }
  return ul;
};
...
netacad.views.CourseIndexView.prototype.buildSectionList = function(sections, filter) {
  ...
  var ul = this.document.createElement("ul");
  ul.setAttribute("class", "sections");
  for (var i = 0; i < sections.length; i++) {
    // <li><span class="section-title">1.2 Section Title</span>...</li>
    var section = sections[i];
    if (section && filter(section)) {
      var li = this.document.createElement("li");
      var span = this.document.createElement("span");
      span.setAttribute("class", "section-title");
      span.appendChild(this.document.createTextNode(section.li + " " + section.title));
      li.appendChild(span);
      li.appendChild(this.buildTopicList(section.children, filter));
      ul.appendChild(li);
    }
  }
  return ul;
...
netacad.views.CourseIndexView.prototype.buildTopicList = function(topics, filter) {
  // <ul class="topics">...</ul>
  var ul = this.document.createElement("ul");
  ul.setAttribute("class", "topics");
  for (var i = 0; i < topics.length; i++) {
    // <li><span class="topic-title">1.2.3 Topic Title</span>...</li>
    var topic = topics[i];
    if (topic && filter(topic)) {
      var li = this.document.createElement("li");
      var span = this.document.createElement("span");
      span.setAttribute("class", "topic-title");
      span.appendChild(this.document.createTextNode(topic.li + " " + topic.title));
      li.appendChild(span);
      li.appendChild(this.buildPageList(topic.children, filter));
      ul.appendChild(li);
    }
  }
  return ul;
...
netacad.views.CourseIndexView.prototype.buildPageList = function(pages, filter) {
  ...
  for (var i = 0; i < pages.length; i++) {
    // <li><a class="page-title" href="#1.2.3.4>1.2.3.4 Page Title</a></li>
    var page = pages[i];
    if (page && filter(page)) {
      var li = this.document.createElement("li");
      var anchor = netacad.components.Gestures.enable(this.document.createElement("a"));
...
netacad.views.BackgroundsView.prototype.build = function() {
  ...
  for (var i = 0; i < backgrounds.length; i++) {
    /*
     * <li>
     *   <img src="course/common-ext/styles/images/backgrounds/ioe-bg-name.jpg"/>
     * </li>
     */
    var background = backgrounds[i];
    ...
    image.setAttribute("data-ident", background);
    image.setAttribute("src", "course/common/backgrounds/" + background + "-S.jpg");
...
netacad.views.LanguagesView.prototype.build = function() {
  ...
  for (var i = 0; i < anchors.length; i++) {
    ...
    var url = netacad.config.languages[lang];
    if (url) {
      anchor.addEventListener("click", this);
    } else {
...
netacad.views.PageView.prototype.setMedia = function(media) {
  ...
  if (length > 1) {
    ...
    this.slideButtons.appendChild(legend);
    this.slideButtons.appendChild(list);
 ...
 * @constructor
 */
netacad.views.SplashView = function(model, element) {
  netacad.views.View.call(this, model, element);
  var that = this;
  $(this.element).addClass("loading");
...
netacad.views.SplashView.prototype.build = function() {
  var images = this.element.querySelectorAll("td img");
  for (var i = 0; i < images.length; i++) {
    var image = images[i];
    image.addEventListener("click", this);
  }
...
netacad.views.SplashView.prototype.handleEvent = function(event) {
  if (event.type == "click") {
    if (event.target instanceof HTMLImageElement) {
      var id = event.target.getAttribute("data-ident");
      this.model.setBackground(id);
