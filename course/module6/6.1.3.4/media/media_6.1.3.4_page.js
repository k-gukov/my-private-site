...
(ClickActivity = function(obj,xmlNode){
	...
	$(canvasXML).find("element").each(function(index, element) {
	 ...
	 if(_id == 1){
			if($(this).attr("type") == "ACTIVITY" && $(this).attr("slide") ==='S1'){
				var obj = {};
				...
				var act1 = new ClickActivity(obj,this);
				_container.addChild(act1);	
			}
		}
	if(_id == 2){
			if($(this).attr("type") == "ACTIVITY" && $(this).attr("slide") ==='S2'){
				var obj = {};
				for(var i = 0;i<this.attributes.length;i++)
				{
					obj[this.attributes[i].name] = this.attributes[i].value;			
				...
				var act2 = new ClickActivity(obj,this);
				_container.addChild(act2);	
			}
		}	
