...
(ClickActivity = function(obj,xmlNode){
	...
	this.generateDesc = function() {
		...
		console.log(th.currDescID + " @@@@@@@@@");
       // alert('th.currDescID: '+th.currDescID);
	...
	this.displayAnswer = function(obj) {
		//alert('display answer: '+th.bindList.length);
		if(th.bindList.length > 0) {
		...
		$(xmlNode).find('subelement').each(function(){
			...
			switch(type){				
					...
					case "TEXTBOX": 						
						...
						compObj.width = $(this).attr('width') ? $(this).attr('width') : 60;	 
						compObj.height = $(this).attr('height') ? $(this).attr('height') : 205;	 
						compObj.x = $(this).attr('x') ? $(this).attr('x') : 0;		
						compObj.y = $(this).attr('y')? $(this).attr('y') : 0;
	...
	this.getAnswersID = function(str){
		var arr = str.split(','); 
		return arr;
	}
	...
	this.btnClicked = function(e){ 
		...
		var answers=[],index=1;
	  // alert('th.bindList.length: '+th.bindList.length);
		if(th.bindList.length == 0) return false;
		//1) get the btn Object
		var btnObj = th.getButton(e.target.name); 
		 
		if(btnObj.ansId.indexOf(th.currDescID) < 0) 
		 { 
		 	showTextBox(false);
			return false;
		 } 
		//alert('btnObj.ansId.indexOf(","): '+btnObj.ansId.indexOf(','));
		if(btnObj.ansId.indexOf(',') > 0){
			answers = th.getAnswersID(btnObj.ansId);
			index = answers.length; 		
		}else answers[0] = btnObj.ansId;
		...
		for(var i=0;i<index ;i++){
			...
			//alert(i);
			if(th.currDescID === answers[i]){
				ansObj=th.currDescID;
				ansArr1.push(ansObj);
				//3)get the Obj
				...
				th.generateDesc();	
				//alert(answers.length+' ::: '+ansArr1.length);
				textBox.visible = false;
	...
	function removeAns(){
		...
		for(var i=0;i < th.elements.texts.length; i++)
		{
			...
			//alert("coming here::"+th.elements.texts[i].compId);
					$('#'+th.elements.texts[i].compId).remove(); 
					if($('#'+th.elements.texts[i].compId)){
						$('#'+th.elements.texts[i].compId).remove(); 
					}
		}
