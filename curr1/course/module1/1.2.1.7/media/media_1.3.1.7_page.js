...
(DragDropComp = function (obj,xmlNode) {
	...
	th.mouseUpHandler = function(ev)
	{
		...
		{   
			...
			if(mediaStage.mouseInBounds && ev.target.hitTest(dropPoint.x,dropPoint.y))
			{
				...
				//alert("this is ev.target.droptarget::::"+ev.target.dropTarget)
			/*	if(ev.target.dropTarget==null ||ev.target.dropTarget.obj.group!=dragArray[dragid].obj.group)
				{
					alert("coming to else "+ev.target.dropTarget)
					snpbackcnt--;
				}*/
	...
	{	
		...
		if(id == "check")
		{
			...
			{
				//console.log("----------------"+dropArray[i].obj.group);
				...
				if(obj.matchType=="group"){
					...
					}else if(dropArray[i].obj.group!=dragArray[dragId].obj.group )
					{
					...
					else if(dragId != dropArray[i].obj.match )
					{
						//console.log("coming to normal") 
						dropArray[i].showWrong();						
